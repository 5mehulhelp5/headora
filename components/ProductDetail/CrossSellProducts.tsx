...
function CrossSellProducts({ Data }: any) {
  ...
  // If crosssell_products length is 0, don't render the component
  ...
  const handlePrev = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0 ? Data?.crosssell_products?.length - itemsToShow : prevIndex - 1
    );
  };
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === Data?.crosssell_products?.length - itemsToShow ? 0 : prevIndex + 1
    );
  };
...
function configurableFinalPrice(item: any) {
  ...
  return (
    ...
    <div className={styles.relatedProductsContainer}>
      <h3 className={styles.title}>CROSS SELL PRODUCTS YOU MAY LIKE</h3>
